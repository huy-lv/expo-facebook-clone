...
class VideoPlayer extends Component {
  ...
  onPlaybackStatusUpdateHandler(status) {
    ...
    if (!this._isDraggingTimePoint) {
      ...
      if (!isNaN(videoId))
        setCurrentWatchingPosition(status.positionMillis, videoId);
    }
  }
  onGestureEventHandler({ nativeEvent }) {
    ...
    const { setCurrentWatchingPosition, videoId } = this.props;
    if (!isNaN(videoId))
      setCurrentWatchingPosition(nextPositionMillis, videoId);
  }
...
const CurrentTimeText = (props) => {
  ...
  const positions = useSelector(
    (state) => state.watch.currentWatchTimePosition
  );
  const { videoId } = props;
